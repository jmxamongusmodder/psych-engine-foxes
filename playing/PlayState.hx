...
class PlayState extends MusicBeatState
{
	...
	var cambruj:FlxSprite;
	var eddbruj:FlxSprite;
	var springbunnymanthing:FlxSprite;
	...
	public static var isEasterButNoPeople:Bool = false;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'easterformitty':
			{
				curStage = 'easterformitty';

				defaultCamZoom = 0.80;

				bg = new FlxSprite(-1000, -500).loadGraphic(Paths.image('easta/bgWalls'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				...
				cambruj = new FlxSprite((-300, 140);
				cambruj.frames = Paths.getSparrowAtlas('easta/bgcam');
				cambruj.animation.addByPrefix('bop', "CamIdle", 24, true);
				cambruj.animation.addByPrefix('bryj', "CamIdle", 24, false);
				cambruj.antialiasing = true;
				cambruj.scrollFactor.set(0.33, 0.33);
				cambruj.setGraphicSize(Std.int(upperBoppers.width * 0.85));
				cambruj.updateHitbox();

				var bgEscalator:FlxSprite = new FlxSprite(-1100, -600).loadGraphic(Paths.image('easta/bgEscalator'));
				bgEscalator.antialiasing = true;
				bgEscalator.scrollFactor.set(0.3, 0.3);
				bgEscalator.active = false;
				bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
				...
				var tree:FlxSprite = new FlxSprite(370, -250).loadGraphic(Paths.image('easta/christmasTree'));
				tree.antialiasing = true;
				tree.scrollFactor.set(0.40, 0.40);

				var fgSnow:FlxSprite = new FlxSprite(-600, 700).loadGraphic(Paths.image('easta/fgSnow'));
				fgSnow.active = false;
				fgSnow.antialiasing = true;
				add(cambruj);
				add(bg);
